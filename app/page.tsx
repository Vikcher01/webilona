...
export default function WebinarLanding() {
  const [showRegistrationForm, setShowRegistrationForm] = useState(true) // Set to true to show on load
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    try {
      // Если сервер успешно выполнит redirect(), управление сюда не вернётся.
      await submitRegistration(formData)
    } catch (error: any) {
      // Игнорируем исключение-редирект от Next.js
